...
class InstallerApp(tk.Tk):
    def __init__(self):
        ...
        # Premium Palette (Google Blue and Clean Grays)
    ...
    def show_welcome_page(self):
        ...
        # Left Blue side banner
        banner = tk.Frame(self.current_frame, bg=self.primary_color, width=150)
        banner.pack(side="left", fill="y")
        
        # Title inside banner (vertical feel)
        lbl_banner = tk.Label(banner, text="IMPORT\nTO\nSHEET", fg="#ffffff", bg=self.primary_color, font=("Segoe UI", 16, "bold"))
        ...
        title = tk.Label(content, text="Chào mừng đến với Trình Cài đặt", fg=self.primary_color, bg=self.bg_color, font=("Segoe UI", 16, "bold"), anchor="w")
        ...
        desc = tk.Label(content, text=(
            ...
            "Chương trình sẽ được cài đặt cục bộ cho tài khoản Windows hiện tại của bạn và không yêu cầu quyền Admin hệ thống.\n\n"
            "Bấm Tiếp tục để bắt đầu."
        ), bg=self.bg_color, font=("Segoe UI", 10), justify="left", wraplength=400)
    ...
    def show_directory_page(self):
        ...
        title = tk.Label(content, text="Chọn Thư mục Cài đặt", fg=self.primary_color, bg=self.bg_color, font=("Segoe UI", 14, "bold"), anchor="w")
        title.pack(fill="x", pady=(0, 10))
        
        desc = tk.Label(content, text="Hãy chọn thư mục trên máy tính của bạn nơi chương trình sẽ được cài đặt.", bg=self.bg_color, anchor="w")
        desc.pack(fill="x", pady=(0, 15))
        
        # Directory input block
        ...
        self.create_navigation_bar(content, self.show_credentials_page, self.show_welcome_page)

    # --- 3. GOOGLE CREDENTIALS PAGE ---
    def show_credentials_page(self):
        ...
        title = tk.Label(content, text="Cấu hình Google Sheets API (credentials.json)", fg=self.primary_color, bg=self.bg_color, font=("Segoe UI", 14, "bold"), anchor="w")
        title.pack(fill="x", pady=(0, 10))
        ...
        ), bg=self.bg_color, justify="left", wraplength=550, anchor="w")
        desc.pack(fill="x", pady=(0, 15))
        
        # Link to open Google Cloud Console
        def open_google_console():
            webbrowser.open("https://console.cloud.google.com/apis/credentials")
            
        btn_console = ttk.Button(content, text="🌐 Mở Google Cloud Console (Để lấy file credentials.json)", command=open_google_console)
    ...
    def run_installation(self):
        ...
        title = tk.Label(content, text="Đang tiến hành cài đặt...", fg=self.primary_color, bg=self.bg_color, font=("Segoe UI", 14, "bold"), anchor="w")
        title.pack(fill="x", pady=(0, 10))
        ...
        self.status_lbl = tk.Label(content, text="Đang khởi tạo các tiến trình cài đặt...", bg=self.bg_color, font=("Segoe UI", 10, "italic"), anchor="w")
    ...
    def step_install(self):
        ...
        # 2. Extract and copy core executable
        ...
        if os.path.exists(src_exe):
            ...
            dummy_file = os.path.join(dest_dir, "ImportToSheet.exe")
            with open(dummy_file, "w") as f:
                f.write("DUMMY EXE - COMPILED OUTPUT NOT FOUND IN RESOURCES")
            
        # 3. Copy credentials.json if uploaded
        ...
        # We write a self-deleting uninstaller batch file
...
"""
        ...
        # 6. Add to Windows Add/Remove Programs (HKCU)
    ...
    def show_finished_page(self, register_success):
        ...
        title = tk.Label(content, text="Cài Đặt Hoàn Tất!", fg="#0f9d58", bg=self.bg_color, font=("Segoe UI", 16, "bold"), anchor="w") # Success Green
        title.pack(fill="x", pady=(0, 10))
        ...
        if register_success:
            msg += "Tùy chọn 'Import to Google Sheets' đã khả dụng khi bạn click chuột phải vào các tệp tin Word (.docx) hoặc Excel (.xlsx, .csv).\n\n"
        else:
            msg += "CẢNH BÁO: Đăng ký registry gặp lỗi, tùy chọn chuột phải có thể không hiển thị đúng.\n\n"
            
        if not self.credentials_path.get():
            msg += (
                "LƯU Ý QUAN TRỌNG: Bạn chưa cung cấp file 'credentials.json'.\n"
                "Trước khi sử dụng, bạn PHẢI copy file 'credentials.json' từ Google API của bạn "
                "vào thư mục cài đặt nêu trên."
            )
